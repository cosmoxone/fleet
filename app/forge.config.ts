...
let cfg = {
  ...
  extendInfo: {
    ...
    NSMicrophoneUsageDescription:
      'Fleet needs access to your microphone for voice dictation.',
    NSAppleEventsUsageDescription:
      'Fleet needs access to send Apple Events to control other apps on your behalf.',
  },
};
...
module.exports = {
  ...
  publishers: [
    {
      ...
      config: {
        repository: {
          owner: process.env.GITHUB_OWNER || 'fleet',
          name: process.env.GITHUB_REPO || 'fleet',
        },
        prerelease: false,
        draft: true,
      },
    ...
    },
    {
      ...
      config: {
        options: {
          id: 'org.fleet.Fleet',
          categories: ['Development'],
          mimeType: ['x-scheme-handler/fleet'],
          icon: {
  ...
  plugins: [
    ...
    new FusesPlugin({
      ...
      // Fleet fork: cross-packing darwin/arm64 from Linux — no codesign binary
      // exists there, so the plugin's ad-hoc signature reset would crash. Keep
      // upstream behavior on macOS; skip only on Linux (the .app is re-signed
      // with `codesign --force --deep -s -` on the target Mac afterwards).
      resetAdHocDarwinSignature: process.platform !== 'linux',
    }),
  ],
};
