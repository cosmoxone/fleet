...
const MENU_TRANSLATIONS_ZH_CN: Record<string, string> = {
  ...
  // Goose-added items
  'New Window': '新建窗口',
  'New Chat on Node…': '在节点上新开聊天…',
  'Add nodes in Settings → Sharing → Fleet Nodes': '在 设置 → 共享 → Fleet Nodes 中添加节点',
  Settings: '设置',
  'Find…': '查找…',
  'Find Next': '查找下一个',
  'Find Previous': '查找上一个',
  ...
  'Focus Fleet Window': '聚焦 Fleet 窗口',
  'Quick Launcher': '快速启动器',
  'Always on Top': '窗口置顶',
  'Toggle Navigation': '切换导航',
  'About Fleet': '关于 Fleet',
...
// Settings management
// =======================================================================
// Fleet fork: per-instance userData via env (multi-instance support).
// Must run before any app.getPath('userData') call below resolves.
// =======================================================================
...
function getSettings(): Settings {
  if (fsSync.existsSync(SETTINGS_FILE)) {
    ...
    return {
      ...
      externalBackends: stored.externalBackends ?? defaultSettings.externalBackends,
      keyboardShortcuts: {
...
if (MAIN_WINDOW_VITE_DEV_SERVER_URL) {
  ...
  console.log('[Main] Development mode: Forcing protocol registration for fleet://');
  app.setAsDefaultProtocolClient('fleet');

  if (process.platform === 'darwin') {
    try {
      // Reset the default handler to ensure dev version takes precedence
      spawn('open', ['-a', process.execPath, '--args', '--reset-protocol-handler', 'fleet'], {
        detached: true,
        stdio: 'ignore',
      });
    } catch {
...
if (process.platform !== 'darwin') {
  ...
  if (!gotTheLock) {
    ...
    app.on('second-instance', (_event, commandLine) => {
      const protocolUrl = commandLine.find((arg) => arg.startsWith('fleet://'));
      if (protocolUrl) {
  ...
  const protocolUrl = process.argv.find((arg) => arg.startsWith('fleet://'));
  if (protocolUrl) {
    app.whenReady().then(async () => {
      let parsedUrl: URL;
      try {
...
async function createResumeChatWindow(parsedUrl: URL, dir?: string): Promise<boolean> {
  ...
  if (!resumeSessionId) {
    log.warn('[Main] Ignoring fleet://resume URL without a session id');
    return false;
  }
...
interface CreateChatOptions {
  ...
  /** Fleet node id from settings.externalBackends: bind this window to that node. */
  backendId?: string;
}
...
const createChat = async (
  ...
  try {
    if (options.backendId) {
      externalBackend = getFleetNodeBackend(settings, options.backendId);
      if (!externalBackend) {
        dialog.showMessageBoxSync({
          type: 'error',
          title: 'Fleet Node Not Found',
          message: `No fleet node with id '${options.backendId}' in settings.`,
          detail: 'Add or fix the entry under "externalBackends" in settings.json, then try again.',
          buttons: ['OK'],
        });
        return;
      }
  ...
  if (externalBackend?.certFingerprint) {
    ...
    const isFleetNode = Boolean(externalBackend.fleetNodeId);
    const usesHttps = (() => {
      try {
    ...
    if (!usesHttps) {
      const response = dialog.showMessageBoxSync({
        ...
        buttons: isFleetNode ? ['OK'] : ['Disable External Backend & Retry', 'Quit'],
        defaultId: 0,
        cancelId: isFleetNode ? 0 : 1,
      });

      if (!isFleetNode && response === 0) {
        updateSettings((s) => {
          if (s.externalGoosed) {
  ...
  if (externalBackend) {
    ...
    try {
      ...
      if (!externalBackendReady) {
        ...
        const isFleetNode = Boolean(externalBackend.fleetNodeId);
        const canDisableExternalBackend = externalBackend.source === 'settings' && !isFleetNode;
        const response = dialog.showMessageBoxSync({
          type: 'error',
  ...
  if (externalBackend?.fleetNodeName) {
    const nodeTitle = `${externalBackend.fleetNodeName} — Fleet`;
    mainWindow.setTitle(nodeTitle);
...
ipcMain.handle('set-setting', (_event, key: SettingKey, value: unknown) => {
  ...
  if (key === 'externalBackends') {
    installFleetMenuItems();
  }
...
// =======================================================================
// Fleet: "New Chat on Node…" menu entry, rebuilt from settings so node list
// changes (settings UI / set-setting IPC) apply without an app restart.
// The whole application menu template is rebuilt because Electron provides
// no API to remove items from a live native menu; conversion preserves
// roles, accelerators, and click handlers of existing items.
// =======================================================================

const menuItemToTemplate = (item: MenuItem): MenuItemConstructorOptions => {
  ...
  if (item.accelerator) {
    template.accelerator = item.accelerator;
  }
  if (item.role) {
    template.role = item.role as NonNullable<MenuItemConstructorOptions['role']>;
  }
  if (typeof item.click === 'function') {
    const click = item.click;
    template.click = (clickedItem, focusedWindow, event) => {
      click(clickedItem, focusedWindow ?? undefined, event);
    };
  }
  if (item.submenu) {
    template.submenu = (item.submenu as Menu).items.map(menuItemToTemplate);
  }
  return template;
};

const installFleetMenuItems = (): void => {
  try {
    const currentMenu = Menu.getApplicationMenu();
    if (!currentMenu) {
      return;
    }
    const fleetLabel = menuT(FLEET_MENU_LABEL);
    const nodes = getSettings().externalBackends;

    const convert = (items: MenuItem[]): MenuItemConstructorOptions[] =>
      items.filter((item) => item.label !== fleetLabel).map(menuItemToTemplate);

    const template = convert(currentMenu.items);
    const fileLabel = menuT('File');
    const gooseLabel = menuT('Fleet');
    const targetEntry =
      template.find(
        (entry) =>
          (entry.label === 'File' || entry.label === fileLabel) && Array.isArray(entry.submenu)
      ) ??
      template.find(
        (entry) =>
          (entry.label === 'Fleet' || entry.label === gooseLabel) && Array.isArray(entry.submenu)
      );
    if (!targetEntry || !Array.isArray(targetEntry.submenu)) {
      return;
    }
    const submenu = targetEntry.submenu as MenuItemConstructorOptions[];
    const built = buildFleetNodeSubmenu(nodes, (node) => {
      void createChat(app, { backendId: node.id, dir: node.workingDir });
    });
    const fleetItem: MenuItemConstructorOptions = {
      ...built,
      label: fleetLabel,
      submenu: Array.isArray(built.submenu)
        ? built.submenu.map((entry) =>
            entry.label === FLEET_MENU_EMPTY_HINT
              ? { ...entry, label: menuT(FLEET_MENU_EMPTY_HINT) }
              : entry
          )
        : built.submenu,
    };
    const newChatLabels = ['New Chat', 'New Chat Window', 'New Window'].map(menuT);
    const newChatIndex = submenu.findIndex(
      (entry) =>
        /New (Window|Chat)/.test(entry.label ?? '') || newChatLabels.includes(entry.label ?? '')
    );
    submenu.splice(newChatIndex >= 0 ? newChatIndex + 1 : 1, 0, fleetItem);
    Menu.setApplicationMenu(Menu.buildFromTemplate(template));
  } catch (error) {
    log.error('Failed to install fleet menu items', error);
  }
};
...
async function appMain() {
    ...
    callback({
      responseHeaders: {
        ...
        'Content-Security-Policy': buildCSP(
          ...
          ...getFleetCspBackends(currentSettings)
        ),
      },
    });
  });
  ...
  const appMenu = menu?.items.find((item) => item.label === 'Fleet');
  if (appMenu?.submenu) {
  ...
  // Fleet: per-node chat windows from the native menu (main-process only feature).
  // Rebuilds the application menu so the node list is always current (called again
  // whenever settings.externalBackends changes via the set-setting IPC).
  ...
  if (fileMenu?.submenu) {
    ...
    if (shortcuts.focusWindow) {
      fileMenu.submenu.append(
        new MenuItem({
          label: menuT('Focus Fleet Window'),
  ...
  if (menu && process.platform !== 'darwin') {
    ...
    if (helpMenu.submenu) {
      ...
      // Create the About Fleet menu item with a submenu
      const aboutGooseMenuItem = new MenuItem({
        label: menuT('About Fleet'),
        submenu: Menu.buildFromTemplate([]), // Start with an empty submenu for About
      });

      // Add the Version menu item (display only) to the About Fleet submenu
  ...
  // Fleet: per-node chat windows from the native menu (main-process only feature).
  // Installed after the translated menu replaces the default one, otherwise the
  // setApplicationMenu call above would wipe the fleet entry. Rebuilt again
  // whenever settings.externalBackends changes via the set-setting IPC.
  installFleetMenuItems();
...
app.whenReady().then(async () => {
  try {
    ...
    dialog.showErrorBox('Fleet Error', `Failed to create main window: ${error}`);
    app.quit();
  }
});
