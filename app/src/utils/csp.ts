...
export function buildConnectSrc(...backends: (ExternalBackendConfig | undefined)[]): string {
  const sources = [...DEFAULT_CONNECT_SOURCES];

  for (const externalBackend of backends) {
...
export function shouldUpgradeInsecureRequests(
  ...backends: (ExternalBackendConfig | undefined)[]
): boolean {
  for (const externalBackend of backends) {
    if (!externalBackend?.enabled || !externalBackend.url) {
      continue;
    ...
    try {
      if (new URL(externalBackend.url).protocol === 'http:') {
        return false;
      }
    } catch {
      // Invalid URL: keep the default secure behavior.
    }
  }
  return true;
}

export function buildCSP(...backends: (ExternalBackendConfig | undefined)[]): string {
  const connectSrc = buildConnectSrc(...backends);
  const upgradeDirective = shouldUpgradeInsecureRequests(...backends)
