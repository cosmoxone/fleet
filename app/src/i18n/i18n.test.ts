...
describe('getLocale', () => {
  ...
  it('maps Simplified Chinese aliases to zh-CN', () => {
    vi.stubGlobal('navigator', { languages: ['zh-Hans-CN'] });
    expect(getLocale()).toEqual({ locale: 'zh-CN', messageLocale: 'zh-CN' });
  });

  it('supports explicit zh-CN locale', () => {
    mockAppConfig({ GOOSE_LOCALE: 'zh-CN' });
    vi.stubGlobal('navigator', { languages: ['xx-XX'] });
    expect(getLocale()).toEqual({ locale: 'zh-CN', messageLocale: 'zh-CN' });
  });

  it('maps Traditional Chinese aliases to zh-TW', () => {
    vi.stubGlobal('navigator', { languages: ['zh-Hant-TW'] });
    expect(getLocale()).toEqual({ locale: 'zh-TW', messageLocale: 'zh-TW' });
  });
